...
int open_sim_dll(const char *sim_dll_path,struct SimDllApi *api_ptrs) {
   ...
   (*api_ptrs).initialize_simulator = (void (*)(const char*)) dlsym(my_sim_lib,"initialize_simulator");
   ...
   (*api_ptrs).simulator_load_elf = (int (*)(int, const char*)) dlsym(my_sim_lib,"simulator_load_elf");
   if(CheckSimOp("simulator_load_elf"))
     return -1;
   ...
   (*api_ptrs).read_simulator_register = (int (*)(int, const char*, uint8_t*, int)) dlsym(my_sim_lib,"read_simulator_register");
   if(CheckSimOp("read_simulator_register"))
     return -1;
   ...
   (*api_ptrs).write_simulator_register = (int (*)(int, const char*, const uint8_t*, int)) dlsym(my_sim_lib,"write_simulator_register");
   ...
   (*api_ptrs).write_simulator_memory = (int (*)(int, const uint64_t*, int, const uint8_t*)) dlsym(my_sim_lib,"write_simulator_memory");
   if(CheckSimOp("write_simulator_memory"))
     return -1;

   (*api_ptrs).translate_virtual_address = (int (*)(int, const uint64_t*, int, uint64_t*, uint64_t*)) dlsym(my_sim_lib,"translate_virtual_address");
   if(CheckSimOp("translate_virtual_address"))
     return -1;

   (*api_ptrs).initialize_simulator_memory = (int (*)(int, const uint64_t*, int, uint64_t)) dlsym(my_sim_lib,"initialize_simulator_memory");
   if(CheckSimOp("initialize_simulator_memory"))
     return -1;
